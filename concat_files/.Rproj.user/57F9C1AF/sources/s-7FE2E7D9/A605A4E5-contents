...
for (folder in folders){
  ...
  if(str_detect(folder,'DNN')){
    for (i in 1:length(files)) {
      ...
      output_filename = paste(curr_fname, ".csv")
      file_contents = read.csv(file_path, skip = num_lines_to_skip)
      result <- data.frame(matrix(ncol = 20, nrow = 190))
      colnames(result) <- c('RT','img_1','img_2','isSameProffesion','img1proffesion','img2proffesion','img1Gender','img2Gender',
                                   ...
                                   "isFamiliar_img1",	"isFamiliar_img2","reject_trial",	"condition","pair")
  ...
  familiarity_output_filename = paste(folder, "_familiarty.csv")
  ...
  for (i in 1:length(files)) {
    ...
    ##for egozi files only:
    # colnames(file_contents) <- c('RT','img_1','img_2','isSameProffesion',	'img1proffesion','img2proffesion','img1Gender',	'img2Gender',	'keyPressed')
    # file_contents$participant = i
    # file_contents$time = 12345
    ...
    # familiarity_data$participant = i
    # familiarity_data$time = 12345
    ...
    #finally, if a subject pressed the same button 95% or more of the trials, reject all his trials:
    # frequncies = table(file_contents$keyPressed)
    # if (sum(frequncies >= 0.95*nrow(file_contents)) > 0){
    #   file_contents$reject_trial = TRUE
    # }
